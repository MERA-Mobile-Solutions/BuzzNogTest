...
void s_leaf(CGContextRef ctx, double t, CGFloat x, CGFloat y, CGFloat cw, CGFloat s, CGColorRef color)
{
  double a = cw / 8,
  b = a / 3,
  c = 2 * b,
...
void swoosh(CGContextRef ctx, double t, CGFloat cx, CGFloat cy, CGFloat cw, CGFloat s, NSInteger index, double total, CGColorRef color)
{
  ...
  if (e < 1) {
    ...
    s_leaf(ctx, t, cx + (path[(int)f - 2] * (1 - e) + path[(int)f] * e) * cw, cy + (path[(int)f - 1] * (1 - e) + path[(int)f + 1] * e) * cw, cw, s, color);
  }
}
